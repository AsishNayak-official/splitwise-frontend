...
import { addExpenseToGroup } from "@/api/groupApi";
import {
  GroupExpenseFormValues,
  groupExpenseSchema,
} from "@/lib/validations/addExpense.validation";
...
const AddGroupExpense = ({ groupId, groupName }: AddGroupExpenseProps) => {
  const [submitError, setSubmitError] = useState<string | null>(null);
  const friendList = useAppSelector((state) => state.friend.friendList);
  const router = useRouter();
  const pathname = usePathname();
  const formik = useFormik<GroupExpenseFormValues>({
    initialValues: {
      groupId: groupId,
      description: "",
      totalAmount: 0,
    ...
    validate: (values) => {
      const result = groupExpenseSchema.safeParse(values);
      if (result.success) return {};
      const errors: FormikErrors<GroupExpenseFormValues> = {};

      result.error.issues.forEach((issue) => {
        const field = issue.path[0] as keyof GroupExpenseFormValues;
        if (!errors[field]) {
